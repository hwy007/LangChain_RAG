...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    try:
        saved_files = FileService.save_upload_files(files, filenames)
        return {"filenames": filenames, "messages": f"Successfully uploaded {len(files)} files."}
...
async def chat(request: ChatRequest):
    """
    Agentic RAG对话接口
    如果提供了 kb_name，Agent将尝试使用检索工具。
    返回结果包括 answer 和 sources (如果有检索)
    """
    try:
